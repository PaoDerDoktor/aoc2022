        ...
        for turn in range(10000):
            for monkey in range(len(inventories)):
                ...
                while objPointer != -1:
                    ...
                    worryLevel: int =  execute_operation(inventories[monkey].pop(), operations[monkey])
                    if worryLevel % tests[monkey][0] == 0:
                        inventories[tests[monkey][1]].insert(0, worryLevel % modulo)
                    else:
                        inventories[tests[monkey][2]].insert(0, worryLevel % modulo)
                    objPointer -= 1
